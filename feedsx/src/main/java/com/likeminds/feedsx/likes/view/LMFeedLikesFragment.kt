...
class LMFeedLikesFragment : BaseFragment<LmFeedFragmentLikesBinding, LikesViewModel>() {
    ...
    override fun receiveExtras() {
        super.receiveExtras()
        if (arguments == null || arguments?.containsKey(LMFeedLikesActivity.LIKES_SCREEN_EXTRAS) == false) {
            requireActivity().supportFragmentManager.popBackStack()
            return
        }
        likesScreenExtras = ExtrasUtil.getParcelable(
            arguments,
            LMFeedLikesActivity.LIKES_SCREEN_EXTRAS,
    ...
    private fun setTotalLikesCount(totalLikes: Int) {
        val likesActivity = requireActivity() as LMFeedLikesActivity
